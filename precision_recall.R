...
calculate_precision_recall_for_each_row<-function(k, D, i, D_euc){
  ...
  d_euc=D_euc[i, ]
  indices_euc=seq(1, length(d_euc))
  #create a dataframe
  df_euc = data.frame(indices_euc, d_euc)
  colnames(df_euc) <- c("ind", "dist")
  #sort dataframe based on distance
  df_sorted_euc <- df_euc[order(df_euc$dist),]
  #select the indices corresponding to the least k distances
  k_nn_index_list_euc <- subset(df_sorted_euc, ind!=i)[1:k,1]
  
  rele_ret = length(intersect(k_nn_index_list, k_nn_index_list_euc))
...
p_r_list<-function(D, D_euc, at_K, num_of_points){
  precision_list=c()
  recall_list=c()
  ...
  for (k in at_K){
    ...
    while(i <= num_of_points){
      ...
      precision_and_recall = calculate_precision_recall_for_each_row(k, D, i, D_euc)
