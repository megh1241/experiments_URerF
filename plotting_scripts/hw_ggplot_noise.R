...
load("rotation_linear_euc_dist_small.Rdata")
load("rotation_linear_varying_dims_with_adelerf_normalized.Rdata")
#load("rotation_linear_varying_dims_with_adelerf.Rdata")
...
load("normalization_mog_euc_dist_small.Rdata")
load("normalization_mog_varying_dims_with_adelerf_normalized.Rdata")
...
precision_df_11=rbind(precision_df11, precision_df22, precision_df33, precision_df44, precision_df55)
p1 <- ggplot(precision_df_11, aes(noise_dims, prec, colour = Algorithm))  +
    ...
    geom_point(alpha=0.9, show.legend = FALSE, legend.position = c(0.8, 0.2))  + xlab('Noise Dimensions') + ylab(NULL)+scale_y_continuous(breaks=seq(0, 1, 0.2), labels=NULL) + scale_color_brewer(palette="Dark2") + scale_x_continuous(breaks=seq(0, 10, 2))

load("rotation_sphere_euc_dist_small.Rdata")
load("rotation_sphere_varying_dims_with_adelerf_normalized.Rdata")
...
precision_df_111=rbind(precision_df111, precision_df222, precision_df333, precision_df444, precision_df555)
p4 <- ggplot(precision_df_111, aes(noise_dims, prec, colour = Algorithm))  +
    ggtitle("Sphere") +
    geom_line(alpha=0.9, show.legend = FALSE, legend.position = c(0.8, 0.2)) +
    geom_point(alpha=0.9, show.legend = FALSE, legend.position = c(0.8, 0.2))  + xlab('Noise Dimensions') + ylab(NULL) +  scale_y_continuous(breaks=seq(0, 1, 0.2), labels=NULL) +  scale_color_brewer(palette="Dark2") + scale_x_continuous(breaks=seq(0, 10, 2))#+ theme_grey(base_size =17)

load("normalization_helix_euc_dist_small.Rdata")
load("normalization_helix_varying_dims_with_adelerf_normalized.Rdata")
...
precision_df_df2=rbind(precision_df7, precision_df8, precision_df9, precision_df10, precision_df11)
...
ggsave("final_plots/varying_dim_adele_with_euc_normalized.png")
