...
hw_noise_data=normalizeData(hw_noise_data)
bootstrap_idx = seq(0, 5, 1)

for(i in bootstrap_idx){
print("isomap")

#Sample 900 out of a 1000
idx_sampled = sample.int(999, 900, replace=FALSE)

#@Veronika i think the two lines below might be wrong/ syntactical issue
hw_noise_data_sampled = hw_noise_data[idx_sampled, ]
D_truth_s = D_truth[idx_sampled, ] 
#hw_noise_data_sampled = sample(hw_noise_data, size=900)
# isomap
D_eucd_noise = as.matrix(dist(hw_noise_data_sampled))
iso_dist_noise = as.matrix(isomapdist(D_eucd_noise, k=15))
iso_dist_noise = as.matrix(iso_dist_noise)
D_truth_s = as.matrix(D_truth_s)
D_iso_noise_p_r_list = p_r_list(iso_dist_noise, D_truth_s, at_K, 900)
D_iso_noise_precision_list= D_iso_noise_p_r_list$precisionList

if(i==0)
    D_iso_final = D_iso_noise_precision_list[[1]]
else
    D_iso_final = D_iso_final +  D_iso_noise_precision_list[[1]]
print(D_iso_noise_precision_list)
}
D_iso_final = D_iso_final/length(bootstrap_idx)
iso_prec_list = c(iso_prec_list, D_iso_final)
...
a_noise = umap(as.matrix(hw_noise_data_sampled), config = custom.settings)
D_umap_noise= as.matrix(dist(a_noise$layout))
D_truth_s = as.matrix(D_truth_s)
D_umap_noise_p_r_list = p_r_list(D_umap_noise, D_truth_s, at_K, 900)
D_umap_noise_precision_list= D_umap_noise_p_r_list$precisionList
if(i==0)
    D_umap_final = D_umap_noise_precision_list[[1]]
else
    D_umap_final = D_umap_final +  D_umap_noise_precision_list[[1]]
}
D_umap_final = D_umap_final/length(bootstrap_idx)
umap_prec_list = c(umap_prec_list, D_umap_final)
print(umap_prec_list)
print(iso_prec_list)
...
#URerF norm
g_noise_norm=Urerf(hw_noise_data, trees = 300, Progress = TRUE, splitCrit = "bicfast", normalizeData = FALSE)
W_noise_norm=g_noise_norm$similarityMatrix
D_rf_noise_norm=1-W_noise_norm
D_rf_noise=1-W_noise_norm
# isomap norm
